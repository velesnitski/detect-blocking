...
### Fixed
- **Two conclusions were reported that had never been measured.** Both surfaced from a run whose own output contradicted itself.
...
### Changed
- **More runs by default — everything that is free, safe and self-triggering.** ("Which probes run" is explicitly outside the frozen 1.0 contract, so this is a minor.)
  - **Connection-limit probe is on by default** at a modest `N=8`. It hits only the target (your own endpoint) and the handshakes are concurrent, so a timed default run measured **no added cost** (28 s vs 29 s). Opt out with the new `--no-conn-test`; `--conn-test N` still raises it.
  - **The local-dialerProxy stub now auto-starts** when a config dials through a local desync proxy (ByeDPI / zapret / GoodbyeDPI) that has **no listener**. Previously every tunnel probe failed and the run read as a dead endpoint when the real cause was just that the desync proxy isn't running on this machine. All the existing guards still apply (a live listener is reused, never replaced), the honest "THROWAWAY PLAIN socks — validates carriage, NOT desync efficacy" warning still prints, and every bail-out is silent in auto mode so ordinary configs gain no noise. Opt out with `--no-stub-dialer`.
  - **The origin panel audit now auto-runs** when host-exposure finds a proxy-panel port answering on a **non-CDN** IP — i.e. a real exposed-panel suspicion on that host, which is both a takeover risk and a strong detectability tell. CDN-fronted hosts are unaffected (their alt-ports are the CDN's by design). Opt out with `--no-panel-probe`.
  - **`--full` also enables the alt-port survey** now, making it the genuinely maximal switch while the default run stays quick.
- **Deliberately still opt-in, for stated reasons:** `--censor-sweep` fetches known-censored sites **from the machine running the tool** — defaulting that would make an in-region operator generate traffic to blocked resources from their own IP, so it stays behind `--full` / the explicit flag with its warning; `--scan-covers` makes unsolicited handshakes to third-party domains; `--reveal` would break the frozen share-safety guarantee; `--pcap` needs root/`tcpdump`.

### Added
- **Whitelist-restriction probe (`whitelist`, on by default).** Some mobile paths permit only a curated set of destinations (the restricted / zero-balance state), so a VPN fails for a reason that has nothing to do with DPI — the whole path is captive, and a stealthier transport cannot help. The probe compares reach of "permitted-class" hosts against neutral controls and classifies via the pure `_classify_whitelist`: `restricted` (permitted-class connect, **all** controls fail → captive path, raises a verdict), `open`, `permitted-unreachable` (controls fine but the permitted-class hosts are not — *not* a restriction, the normal result when probing from outside their region), `no-network`. TCP-only, no HTTP fetch, no new dependencies; the per-host wait is capped at 3 s so a dark network can't stretch a default run. Host lists are public services only, overridable via `WHITELIST_HOSTS` / `WHITELIST_CONTROL_HOSTS`. JSON: `probes.whitelist.{status, permitted_reachable, controls_reachable}`.
- **`--label NAME` — name the vantage.** Stamped into `--json` as top-level `.label` and shown in the baseline-diff header alongside `.environment.default_interface`, so comparing two runs of the *same* endpoint from *different* network paths (e.g. `--label lte-megafon` vs `--label wifi-mgts`) is self-describing instead of anonymous. Without it the diff prints a one-line tip. Pairs with `--save-baseline` / `--diff-baseline`; the label survives the round-trip.
- **`httpupgrade` transport awareness (probe 18).** The CDN-frontable HTTP/1.1-Upgrade transport is now recognized by name instead of falling through the generic branches: it lints `httpupgradeSettings.path` (empty path ⇒ the server answers 404 and the tunnel never establishes — a real lint finding), notes when `.host` is absent (Host falls back to the TLS serverName), and states plainly that it carries HTTP framing (so the FET entropy classifier does not apply) and that XTLS vision cannot be used on it (vision needs raw TCP) — cover-SNI quality and ECH are the levers instead. Behaviour for `ws`/`grpc`/`xhttp` is unchanged.

### Fixed
- **`--localize` claimed a block when probe 2 never ran.** `TCP_OK` initializes to `0`, which was indistinguishable from "probe 2 measured the target unreachable" — so any run that excluded the TCP probe (`--skip tcp`, or an `--only` list without it) auto-started localization and could raise a hard verdict on a perfectly healthy host (reproduced: `--only dns,localize www.example.com` → *"Block localized to the access edge … the filtering is in your local/ISP network"*). Added a `TCP_TESTED` flag; `reachable` is now tri-state (reachable / measured-unreachable / **unknown**), and the pure classifier only claims `access-edge` / `transit` / `near-destination` when unreachability was actually **measured**. Unknown → `incomplete`, with an explicit "probe 2 did not run, so there is no evidence the target is blocked" note. Auto-run now requires measured unreachability; `--localize` still forces a run but reports honestly. Third fix in this family — "not measured" must never read as "measured bad".
- **`LOCALIZE_WAIT` was unvalidated.** `LOCALIZE_WAIT=0` made `traceroute` exit with *"wait time must be > 0"*; the error was swallowed and surfaced as a misleading "no usable hops (ICMP likely rate-limited or filtered)". Now clamped to 1–10 s, mirroring the `LOCALIZE_MAX_HOPS` guard. Both env knobs are documented in `--help`.
