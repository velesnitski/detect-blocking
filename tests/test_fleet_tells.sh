...
# --- A: fake cover + active-probe non-relay + exposed ports (the critical shape) ---
row '{"probes":{"xray_cover":{"status":"fake"},"xray_active_probe":{"matches_cover":false},"xray_tls_parity":{"status":"ok"},"host_exposure":{"open_ports":["22/ssh","8080/http"]},"xray_detectability":{"score":100,"band":"critical","deployment_fingerprint":"abcdef012345","cover_obscure":false,"sni_ip_asn_match":true,"sni_resolves":true,"port_standard":true,"sni_keyword":false,"tls_in_tls_protected":true,"volume_throttle_suspected":false}}}'
...
case "$r_tells" in *no-relay*)    : ;; *) fail "A: tells should include no-relay (got '$r_tells')" ;; esac
case "$r_tells" in *exposed:2*)   : ;; *) fail "A: tells should include exposed:2 (got '$r_tells')" ;; esac
...
# --- E: garbage payload → empty fields (the caller defaults them to ?/-) ---
row 'not json at all'
[ -z "$r_score" ] && [ -z "$r_band" ] || fail "E: garbage payload should yield empty fields (got score='$r_score' band='$r_band')"

echo "PASS: _fleet_row_fields derives score/band/fp and the right tells; clean + garbage degrade safely"
